 ...
 */
package org.fcrepo.camel.indexing.disk.integration;
...
public class RouteUpdateIT extends CamelBlueprintTestSupport {
    ...
    protected Properties useOverridePropertiesWithPropertiesComponent() {
        ...
        props.put("disk.baseDir", "target/rdf");
        props.put("disk.serialization", "TURTLE");
        props.put("disk.extension", ".ttl");
    ...
    public void testAddedEventRouter() throws Exception {
        ...
        context.getRouteDefinition("FcrepoDiskRouter").adviceWith(context, new AdviceWithRouteBuilder() {
            @Override
            public void configure() throws Exception {
                mockEndpoints("*");
            }
        });
        context.getRouteDefinition("FcrepoDiskIndexer").adviceWith(context, new AdviceWithRouteBuilder() {
            @Override
            public void configure() throws Exception {
                mockEndpoints("*");
            }
        ...
        context.getRouteDefinition("FcrepoDiskUpdater").adviceWith(context, new AdviceWithRouteBuilder() {
            @Override
            public void configure() throws Exception {
                mockEndpoints("*");
            }
        ...
        getMockEndpoint("mock://direct:update.disk").expectedMessageCount(1);
        getMockEndpoint(fcrepoEndpoint).expectedMessageCount(2);

        final File f = new File("target/rdf/localhost/" + FCREPO_PORT + "/fcrepo/rest/" + path + ".ttl");
